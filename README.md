...
- 이미지 첨부·인식을 지원합니다. PNG/JPEG/WebP, 이미지당 4 MiB, 요청 전체 8 MiB 및 서로 다른 이미지 4장까지입니다. 대화 내 이미지는 후속 질문에도 다시 전달되므로 이전 첨부도 한도에 포함합니다. 원격 URL과 직접 파일 경로를 받는 HTTP 요청은 거부하며, Codex가 첨부 파일을 변환한 인라인 이미지만 처리합니다. PNG는 최대 32메가픽셀입니다. 파일 헤더·형식·크기를 검증하며 실제 이미지 디코딩은 Grok가 수행합니다.
